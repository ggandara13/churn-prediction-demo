...
def train_all_models_once(_df_hash: str, df: pd.DataFrame):
    """
    Train all model configurations once and cache forever.
    
    Args:
        _df_hash: Hash of dataframe to bust cache if data changes
        df: The dataframe to train on
    
    Returns:
        Dictionary with all trained model configurations
    """
    results = {}
    
    # Train base (no LLM)
    results['base'], _, _, _ = train_models_advanced(
        df, use_llm_features=False, use_smote=False, use_ga=False
    )
    
    # Train +LLM only (BEST model)
    results['llm_only'], _, scaler, _ = train_models_advanced(
        df, use_llm_features=True, use_smote=False, use_ga=False
    )
    
    # Train +LLM+SMOTE
    results['llm_smote'], _, _, _ = train_models_advanced(
        df, use_llm_features=True, use_smote=True, use_ga=False
    )
    
    results['scaler'] = scaler
    results['feature_cols'] = ALL_FEATURES
...
def main():
    ...
    3. Trains models ONCE (cached)
    4. Routes to appropriate page based on sidebar selection
    """
    # Render header
    render_header()
    ...
    # Train all models ONCE at startup (cached forever)
    if page in ["🤖 Model Comparison", "📈 Business Impact", "🎮 Live Prediction"]:
        df_hash = str(len(df)) + str(df['churn'].sum())  # Simple hash
        all_results = train_all_models_once(df_hash, df)
    ...
    if page == "🏠 Overview":
        ...
        page_model_comparison_fast(df)
    elif page == "📈 Business Impact":
        page_business_impact_fast(df)
    elif page == "🎮 Live Prediction":
        page_live_prediction_fast(df)
